...
public class MutualExclusion {
	...
	public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException {
		...
		List<Message> removePeers = removeDuplicatePeersBeforeVoting();

		// multicast the message to activenodes (hint: use multicastMessage)
		multicastMessage(message, removePeers);

		// check that all replicas have replied (permission)
		if(areAllMessagesReturned(removePeers.size())){
	...
	public void onMutexRequestReceived(Message message) throws RemoteException {
		...
		if(message.getNodeIP().equals(node.getNodeName())){
			...
			//Node or not to node - William Shakespare
	...
	public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException {
		...
		switch(condition) {
			...
			case 2: {
				...
				if(sendClock < ownClock || (sendClock == ownClock && message.getNodeID().compareTo(node.getNodeID()) < 0)){
